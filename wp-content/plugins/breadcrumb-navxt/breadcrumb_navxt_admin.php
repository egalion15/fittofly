	...
	function help()
	{
		...
		if($screen->id == 'settings_page_' . $this->identifier)
		{
			$general_tab = '<p>' . __('Tips for the settings are located below select options.', $this->identifier) .
				...
				sprintf(__("%sTutorials and How Tos%s: There are several guides, tutorials, and how tos available on the author's website.", $this->identifier),'<a title="' . __('Перейти the Breadcrumb NavXT tag archive.', 'breadcrumb_navxt') . '" href="http://mtekk.us/archives/tag/breadcrumb-navxt">', '</a>') . '</li><li>' .
				sprintf(__('%sOnline Documentation%s: Check out the documentation for more indepth technical information.', 'breadcrumb_navxt'), '<a title="' . __('Перейти the Breadcrumb NavXT online documentation', 'breadcrumb_navxt') . '" href="http://mtekk.us/code/breadcrumb-navxt/breadcrumb-navxt-doc/">', '</a>') . '</li><li>' .
				sprintf(__('%sReport a Bug%s: If you think you have found a bug, please include your WordPress version and details on how to reproduce the bug.', $this->identifier),'<a title="' . __('Перейти the Breadcrumb NavXT support post for your version.', 'breadcrumb_navxt') . '" href="http://mtekk.us/archives/wordpress/plugins-wordpress/breadcrumb-navxt-' . $this->version . '/#respond">', '</a>') . '</li></ul>' . 
				'<h5>' . __('Giving Back', $this->identifier) . '</h5><ul><li>' .
				sprintf(__('%sDonate%s: Love Breadcrumb NavXT and want to help development? Consider buying the author a beer.', $this->identifier),'<a title="' . __('Перейти PayPal to give a donation to Breadcrumb NavXT.', 'breadcrumb_navxt') . '" href="https://www.paypal.com/cgi-bin/webscr?cmd=_donations&business=FD5XEU783BR8U&lc=US&item_name=Breadcrumb%20NavXT%20Donation&currency_code=USD&bn=PP%2dDonationsBF%3abtn_donateCC_LG%2egif%3aNonHosted">', '</a>') . '</li><li>' .
				sprintf(__('%sTranslate%s: Is your language not available? Contact John Havlik to get translating.', $this->identifier),'<a title="' . __('Перейти the Breadcrumb NavXT translation project.', 'breadcrumb_navxt') . '" href="http://translate.mtekk.us/projects/breadcrumb-navxt">', '</a>') . '</li></ul>';
		...
		<form action="options-general.php?page=breadcrumb_navxt" method="post" id="bcn_admin-options">
			...
			foreach($wp_post_types as $post_type)
			{
				...
				if(!$post_type->_builtin)
				{
					...
					if(!array_key_exists('Hpost_' . $post_type->name . '_template', $this->opt) || !$post_type->hierarchical && !array_key_exists('Spost_' . $post_type->name . '_taxonomy_type', $this->opt))
					{
						...
						$this->opt['Hpost_' . $post_type->name . '_template'] = __('<a title="Перейти %title%." href="%link%">%htitle%</a>', 'breadcrumb_navxt');
			...
			foreach($wp_taxonomies as $taxonomy)
			{
				...
				if(!$taxonomy->_builtin)
				{
					...
					{
						//Add the necessary option array members
						$this->opt['H' . $taxonomy->name . '_template'] = __(sprintf('<a title="Перейти the %%title%% %s archives." href="%%link%%">%%htitle%%</a>', $taxonomy->labels->singular_name), 'breadcrumb_navxt');
	...
	function find_posttypes(&$opts)
	{
		...
		foreach($wp_post_types as $post_type)
		{
			...
			if(!$post_type->_builtin)
			{
				...
				{
					//Add the necessary option array members
					$opts['Hpost_' . $post_type->name . '_template'] = __('<a title="Перейти %title%." href="%link%">%htitle%</a>', 'breadcrumb_navxt');
	...
	function find_taxonomies(&$opts)
	{
		...
		foreach($wp_taxonomies as $taxonomy)
		{
			...
			if(!$taxonomy->_builtin)
			{
				...
				{
					//Add the necessary option array members
					$opts['H' . $taxonomy->name . '_template'] = __(sprintf('<a title="Перейти the %%title%% %s archives." href="%%link%%">%%htitle%%</a>', $taxonomy->labels->singular_name), 'breadcrumb_navxt');
