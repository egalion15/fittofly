			...
			if ( '0' != $current ) {
				...
				if ( $this->subscribe2_options['admin_email'] == 'unsubs' || $this->subscribe2_options['admin_email'] == 'both' ) {
					...
					if ( empty($recipients) ) {
						global $wp_version;
						if ( version_compare($wp_version, '3.1', '<') ) {
							// WordPress version is less than 3.1, use WP_User_Search classE
							if ( !class_exists(WP_User_Search) ) {
								require(ABSPATH . 'wp-admin/includes/user.php');
								$wp_user_query = new WP_User_Search( '', '', 'administrator');
								$admins_string = implode(', ', $wp_user_query->get_results());
...
/* ===== template and filter functions ===== */
	...
	*/
	function shortcode($atts) {
		...
		// build default form
		$this->form = "<form method=\"post\" action=\"" . $url . "\"><input type=\"hidden\" name=\"ip\" value=\"" . $_SERVER['REMOTE_ADDR'] . "\" /><p><label for=\"s2email\">" . __('Наша лента новостей:', 'subscribe2') . "</label><br /><input type=\"text\" name=\"email\" id=\"s2email\" value=\"" . __('введите ваш e-mail', 'subscribe2') . "\" size=\"20\" onfocus=\"if (this.value == '" . __('введите ваш e-mail', 'subscribe2') . "') {this.value = '';}\" onblur=\"if (this.value == '') {this.value = '" . __('введите ваш e-mail', 'subscribe2') . "';}\" /></p><p>" . $this->input_form_action . "</p></form>\r\n";
